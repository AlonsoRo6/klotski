...
def reeval_all(
    ...
    if not eval_files:
        print(f"No s'ha trobat cap .eval.json a {evals_dir}/")
        sys.exit(0)

    print(f"Trobats {len(eval_files)} puzzles ja valorats. Reavalaunt...\n")
    ...
    for eval_path in eval_files:
        ...
        puzzle_name = eval_path.name.replace(".eval.json", ".json")
        puzzle_path = puzzles_dir / puzzle_name
        
        graph_name  = eval_path.name.replace(".eval.json", ".graphml")
